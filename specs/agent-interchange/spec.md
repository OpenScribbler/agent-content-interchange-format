...
The design record carried no drawn agent extension block; the §6.1 schema was drafted at spec-promotion time from the canonical fields the record references — `tools`/`disallowed_tools`, `model`, `mcp_servers`, `skills` (Decisions #23/#28), and `permission_mode`/`background` (named as canonical fields in the panel's deferred items). Field names are the snake_case forms of the referenced struct fields. This schema is to be ratified back into the design record at promotion time.
