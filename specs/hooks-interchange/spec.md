...
**mechanism token** — the closed identifier naming a §7.4 source-mechanism row in a pre-abstracted provider configuration (§7.4).

**recognition** — the ingestion-context step that decides which source mechanism a raw provider configuration uses and produces the mechanism's normalized content; distinct from mapping, which is the canonicalizer's obligation (§7.4).
...
The mapping from observed provider per-OS mechanisms to canonical script entries is total over the surveyed space; anything outside it rejects. This section defines both halves of that contract: the closed mechanism-token vocabulary under which a pre-abstracted provider configuration names its mechanism, and the per-mechanism shape predicates and mappings the canonicalizer applies.

**The two-stage contract.** A provider per-OS mechanism reaches the canonicalizer pre-abstracted: the ingestion context ([ACIF-CORE] §2) supplies a **mechanism token** naming a row of this section, together with the mechanism's content in that row's normalized shape. *Recognition* — deciding which mechanism a raw provider configuration uses and producing the normalized content — is performed by the ingestion context (a conformance harness, a registry's provider recognizer) and is outside the canonicalizer's scope. *Mapping* — everything this section defines — begins at the token boundary. Every mechanism row is **recognition-requiring**: whether a given instance is mapped is decided by the row's shape predicate over the content, never by the token claim alone. A conforming canonicalizer MUST evaluate the recognized row's shape predicate and MUST reject `acif.hook.platform_mechanism_malformed` on violation.

**Mechanism tokens** form the closed set `{per-os-key-map, dual-shell-fields, filename-extension-convention}`, matched by exact byte comparison ([ACIF-CORE] §8.3). `per-os-key-map-provider` is an accepted alias of `per-os-key-map` — it is the render-target name for the same mechanism, and round-trip re-ingestion presents it. Tokens are ACIF-owned recognizer vocabulary; which providers exhibit a given mechanism is observational snapshot data. The token set is machine-projectable: it is published as `conformance/source-mechanisms.yaml` and kept current by the runner selftest's source-mechanisms sync check (CHANGE-PROCESS.md, source-of-truth rule). Tokens beginning `unknown-` are RESERVED for negative test fixtures and are never minted.

**Evaluation order.** A conforming canonicalizer evaluates a pre-abstracted provider configuration in this order; each step binds before the next is considered:

1. **Envelope.** The configuration's content MUST be present and decodable (a structured value, or a string that parses as JSON). Violation → reject `acif.hook.platform_mechanism_malformed`.
2. **Hook-block passthrough.** If the decoded content is an object carrying an `event` member, it is an ordinary hook extension block: it enters §6 ingestion directly, and no mechanism classification occurs.
3. **Token membership.** A mechanism token that is not a member of the closed set above (directly or via the alias) → reject `acif.hook.platform_unmappable` (the totality net). Implementations MUST NOT emit `acif.hook.platform_unmappable` for any input whose mechanism token is a member.
4. **Shape predicate.** The recognized row's shape predicate is evaluated over the decoded content; violation → reject `acif.hook.platform_mechanism_malformed`. An instance satisfying the predicate maps per its row below.

**Shape predicates.** The content of every tokenized mechanism MUST be an object; per token:

- `per-os-key-map` — the keys `windows`, `linux`, `osx` (per-OS entries) and `command` (the base entry) each, when present, MUST carry a string entrypoint path; a non-string value under any of these keys is malformed. The key set is **open**: any other key is opaque passthrough carried on the base entry ([ACIF-CORE] §8.5), so passthrough keys present without a base `command` are malformed — no entry exists to carry them.
- `dual-shell-fields` — the key set is **closed**: a non-empty subset of `{bash, powershell}`, each key carrying a string entrypoint path. Any other key, any non-string value, or an empty object is malformed.
- `filename-extension-convention` — the key `file` is REQUIRED and MUST carry a string path; its absence or a non-string value is malformed.

| Token | Source mechanism | Canonical mapping | Diagnostics / provenance |
|---|---|---|---|
| `per-os-key-map` (alias `per-os-key-map-provider`) | Per-OS key map (`windows` / `linux` / `osx` keys, each with a command) plus a base command | Each per-OS key → a singleton constrained entry (`osx` renamed `darwin` before validation); after the key mapping, constrained entries with identical executable identity (§13.1: `(type, path)` for `type: file`; `(type, SHA-256(content))` for `type: inline`, content normalized per §9.3) MUST be merged into a single entry whose `os` set is the sorted union (§7.1); the base command → the default entry | Lossless; provenance `declared` |
| `dual-shell-fields` | Dual shell fields (`bash` and `powershell` commands) | `bash` → constrained entry `os: [darwin, linux]`; `powershell` → constrained entry `os: [windows]` | MUST emit `acif.hook.platform_shell_os_proxy`; provenance `inferred-from-convention` |
| `filename-extension-convention` | Filename-extension convention | `.ps1` / `.cmd` / `.bat` → `os: [windows]`; `.sh` and extensionless → `os: [darwin, linux]`; any other extension → default entry, MUST emit `acif.hook.platform_filename_uninferable`; successful inference MUST emit `acif.hook.platform_filename_inferred` (INFORMATIVE). Extension comparison is **case-insensitive** (ASCII case folding of the extension only — `hooks/A.PS1` infers `windows`); the path itself is never case-folded and enters canonical form byte-identical | Provenance `inferred-from-convention` |
| *(no dedicated token)* | Single interpreter-selection field on one command (e.g., `shell: powershell`) | Excluded from OS mapping — one entrypoint plus an interpreter flag is nothing to map; the field is carried as opaque passthrough. Reached through ordinary hook-block ingestion (evaluation step 2) or as the base-command/passthrough interior of a `per-os-key-map` instance | See §12.3 (structured encoder); the value participates in the hash preimage (§9) |
| *(no dedicated token)* | Single command, no per-OS mechanism | Default entry. Reached through ordinary hook-block ingestion (evaluation step 2) or as the base command of a `per-os-key-map` instance | — |
| *(any other token)* | Any unmapped mechanism | MUST reject `acif.hook.platform_unmappable` (evaluation step 3) | Totality net |

**The synthesized envelope.** A pre-abstracted provider configuration carries a per-OS script mechanism, not a hook block, so the canonicalizer materializes the surrounding envelope itself — and materializes it deterministically, because `body_hash` covers it ([ACIF-CORE] §8.2 materialize-then-hash). The canonical hook block synthesized around the mapped script entries carries `event: before_tool_execute` and a single `command` handler holding those entries, with every §6.2 default materialized before any hash is computed. Implementations MUST synthesize exactly this envelope; the TV-PLATFORM body-hash literals witness it, and a configuration that should carry a different event reaches canonical form as an ordinary hook block (evaluation step 2), never through mechanism mapping.
...
*(Informative)* The `platform_unmappable`/`platform_mechanism_malformed` split is load-bearing for downstream gap detection: the totality net is the Class B evidence signal of the change process (CHANGE-PROCESS.md), so a malformed instance of an already-mapped mechanism must never masquerade as an unmapped mechanism — the former is a fix-forward authoring error, the latter a candidate spec gap.
...
| `acif.hook.platform_unmappable` | reject | Mechanism token outside the §7.4 token set (evaluation step 3); MUST NOT be emitted for a member token |
