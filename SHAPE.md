...
hook:
  ...
  scripts:                      # required — what the harness executes when the hook fires.
                                # Entries are OS-variants of ONE logical entrypoint (Decision #33):
...
| OQ-9 | **Open** | Two-tier freshness interop — when an external attestation manifest has its own expiry and the sidecar `expires` differ, which takes precedence for staleness determination? **Dependency note (OP-COND-URI-5):** the comparison presumes a stable, re-fetchable `source_uri` — Decision #32's re-crawl stability invariant is a precondition for this question being implementable. |
