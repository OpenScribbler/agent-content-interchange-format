...
| 36 | Verdict-reason identifier mint: 9 identifiers, minted ahead of assertion | The 23 `{conformant: false}` vector sites carried 15 informative free-text reason strings (PROTOCOL §3: "never asserted") — unassertable non-conformance vocabulary. **Minted (class `reject (verdict)`):** `acif.envelope.{kind_invalid, id_invalid, version_invalid, license_spdx_invalid, forbidden_field}` (CORE §5.1/§5.2; `id_invalid` encodes the inferred-pack UUIDv5 exception; `license_spdx_invalid` scoped to malformation only, matching TV-11 case_4 — the absence case is unminted; `forbidden_field` carries required param `field`) and `acif.requires.orphan_key` (CORE §9.4; required param `key`) in CORE §8.7; `acif.registry.{method_stamp_missing, provenance_tag_missing, timestamp_offset_missing}` (§8.3/§8.5/§11.2) in REGISTRY §12. **The orphan_key collapse (6 strings → 1 identifier) is deliberate:** §9.4 defines one uniform reject ("one outcome"); the cause taxonomy (foreign-type, disposed, latent-field-match, considered-and-rejected) is informative design history — minting it would force implementations to carry per-type disposition tables to classify WHY, create ambiguous dual-cause inputs with no disambiguation rule, and let a correct reject fail conformance on a mis-classified cause. Discrimination lives in each vector case's input key plus the `key` param. **Dropped from the proposal (10 → 9):** `acif.registry.staleness_clock_conflation` — it names implementation behavior no conforming adapter can detect and report (TV-FRESH-k's `implementation_behavior` input is a confession field, normative theater); at the flip, TV-FRESH-k reframes to a behavioral `staleness: stale` assertion on divergent clocks. **Sequencing (load-bearing, impl #1 in flight):** minted-ahead-of-assertion — specs gain identifiers + prose mentions now (so impl #1 emits them day-one); vector expects, bindings, selftest untouched; the exact-string flip + `adapter_protocol` bump land after impl #1 merges, gated on the adapter's declared handshake protocol (never retroactive — protects graduation-evidence re-runs). CORE §8.7 carries the enforcement-boundary carve-out so the verbatim-report MUST does not contradict the still-verdict-channel vectors in the gap; PROTOCOL Appendix B maps all 23 vector sites → identifiers (exposing the many-to-one) with an audience note; Appendix A pins the reserved `field`/`key` param shapes now so the detail survives the flip. Interim emit guidance is explicitly informative (a bare normative SHOULD would be untestable under protocol 1). Deferred to the flip pass: runner advisory warning on reason values matching `^acif\.` but ≠ expected (platform-vendor's pre-flip signal; impl #1 can consult Appendix B directly). Mini-review: spec-purist + platform-vendor + karpathy, all ADOPT-WITH-CHANGES, changes applied; adopted under Holden's standing delegation, 2026-07-13. **Test vectors:** existing 23 sites unchanged this pass; assertion flip is the roadmap item. |
| 37 | §7.4 identity-merge after per-OS key mapping (suite-forced disambiguation) | Rendering a multi-OS constrained entry (e.g. `os: [darwin, linux]`) to a per-OS key-map provider necessarily emits the same path under two keys; §7.4's literal mapping ("each per-OS key → a singleton constrained entry") re-ingested those as TWO singleton entries, so §12.3 round-trip identity was unachievable for any multi-OS entry — and TV-PLATFORM-r (`roundtrip_identity: true` through `per-os-key-map-provider`, ratified in the veto skim) already **required** the merge the prose forbade: the suite and the spec disagreed, and under the vectors-win rule the suite had silently decided. Surfaced by syllago Stage 1 (impl #1, 2026-07-13), which implemented the merge to pass. **Resolution (ratifies syllago's behavior):** after the per-OS key mapping, constrained entries with identical executable identity (§13.1: `(type, path)` for `type: file`; `(type, SHA-256(content))` for `type: inline`, content normalized per §9.3) MUST be merged into a single entry whose `os` set is the sorted union (§7.1). Default entries never participate (the sentence scopes to constrained entries). No-op on TV-PLATFORM-i/j (distinct paths per key). Pins the canonical form — and hence `body_hash` — of duplicate-path key maps, which was otherwise implementation-divergent. §12.3 gains an informative note deriving round-trip identity from the merge. **Rejected:** leave §7.4 literal (spec permanently contradicts a published vector; `body_hash` ambiguity across implementations); defer to roadmap (same contradiction, plus impl #2 could conformantly disagree with impl #1 on canonical bytes). No reviewer subset: the suite had already answered the question — this ratifies shipped, vector-forced behavior; closer to defect repair than vocabulary mint. Presented to Holden (AskUserQuestion) and adopted with companion vector, 2026-07-13. **Vector-impact inventory: no existing expectation flips.** Companion vector TV-PLATFORM-i2 (Appendix C (i′)) pins the merged form's `body_hash` via `generate_vectors.py`, so the merge is enforced, not just permitted. Landed alongside (same suite-manifest entry, class `repair`) two suite-defect repairs from the same syllago run: TV-PLATFORM-l case_6 (binding compared the `os: absent` catalog label to the `<absent>` wire sentinel — label↔wire translation added per runner DESIGN §4) and TV-PLATFORM-r (published `canonical_scripts` not in §9.3 canonical order — reordered; dead-default check made structural). |
